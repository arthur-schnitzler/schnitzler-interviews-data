        ...
        <xsl:element name="ncx" namespace="http://www.daisy.org/z3986/2005/ncx/">
            ...
            <xsl:element name="docTitle" namespace="http://www.daisy.org/z3986/2005/ncx/">
                <xsl:element name="text" namespace="http://www.daisy.org/z3986/2005/ncx/">
                    <xsl:text>Arthur Schnitzler: »Das Zeitlose ist von kürzester Dauer.« Interviews, Meinungen, Proteste (1891–1931)</xsl:text>
                </xsl:element>
            </xsl:element>
            ...
            <xsl:element name="navMap" namespace="http://www.daisy.org/z3986/2005/ncx/">
                ...
                <xsl:for-each
                    select="descendant::TEI[starts-with(@id, 'I')]">
                    ...
                    <xsl:element name="navPoint" namespace="http://www.daisy.org/z3986/2005/ncx/">
                        ...
                        <xsl:element name="navLabel"
                            ...
                            <xsl:element name="text"
                                namespace="http://www.daisy.org/z3986/2005/ncx/">
                                <xsl:copy-of select="descendant::titleStmt/title[@level='a']/text()"/>
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="content" namespace="http://www.daisy.org/z3986/2005/ncx/">
                            <xsl:attribute name="src">
                                <xsl:value-of select="concat(@id, '.xhtml')"/>
                            </xsl:attribute>
                        </xsl:element>
                    </xsl:element>
                </xsl:for-each>
                <xsl:for-each
                    select="descendant::TEI[starts-with(@id, 'M')]">
                    <xsl:sort select="@when" order="ascending"/>
                    <xsl:element name="navPoint" namespace="http://www.daisy.org/z3986/2005/ncx/">
                        <xsl:attribute name="id">
                            <xsl:value-of select="@id"/>
                ...
                <xsl:for-each
                    select="descendant::TEI[starts-with(@id, 'P')]">
                    <xsl:sort select="@when" order="ascending"/>
                    <xsl:element name="navPoint" namespace="http://www.daisy.org/z3986/2005/ncx/">
                        <xsl:attribute name="id">
                            <xsl:value-of select="@id"/>
                ...
                <xsl:for-each
                    select="descendant::TEI[starts-with(@id, 'E')]">
                    <xsl:sort select="@when" order="ascending"/>
                    <xsl:element name="navPoint" namespace="http://www.daisy.org/z3986/2005/ncx/">
                        <xsl:attribute name="id">
                            <xsl:value-of select="@id"/>
