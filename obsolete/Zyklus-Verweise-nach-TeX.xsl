...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:key name="place-lookup" match="row" use="Nummer"/>
   ...
   <xsl:function name="foo:person-fuer-index">
      ...
      <xsl:variable name="indexkey"
                    select="key('person-lookup', $xkey, $persons)"
                    as="node()?"/>
      ...
      <xsl:variable name="kVorname"
                    as="xs:string?"
                    select="normalize-space($indexkey/Vorname)"/>
      <xsl:variable name="kZusatz"
                    as="xs:string?"
                    select="normalize-space($indexkey/Zusatz)"/>
      <xsl:variable name="kBeruf"
                    as="xs:string?"
                    select="normalize-space($indexkey/Beruf)"/>
      <xsl:variable name="kGeburtsdatum"
                    as="xs:string?"
                    select="normalize-space($indexkey/Geburtsdatum)"/>
      <xsl:variable name="kGeburtsort"
                    as="xs:string?"
                    select="normalize-space($indexkey/Geburtsort)"/>
      <xsl:variable name="kTodesdatum"
                    as="xs:string?"
                    select="normalize-space($indexkey/Todesdatum)"/>
      <xsl:variable name="kTodesort"
                    as="xs:string?"
                    select="normalize-space($indexkey/Todesort)"/>
      <xsl:choose>
   ...
   <xsl:function name="foo:werk-in-index">
      ...
      <xsl:variable name="zyklus-entry"
                    select="key('work-lookup', substring($work-entry/Zyklus,1, 7), $works)"/>
      <xsl:choose>
  ...
  <!-- Dient dazu, in der Kopfzeile »März 1890« erscheinen zu lassen -->
   ...
   <xsl:function name="foo:sectionInToc">
      ...
      <xsl:variable name="titelminusdatum"
                    as="xs:string"
  ...
  <xsl:template match="back"/>
   ...
   <xsl:template match="date">
    ...
    <xsl:function name="foo:monogr-angabe">
       <xsl:param name="monogr" as="node()"/>
       <xsl:param name="vor-dem-at" as="xs:boolean"/> 
      <!-- Der Parameter ist gesetzt, wenn auch der Sortierungsinhalt vor dem @ ausgegeben werden soll -->
       <xsl:choose>
          ...
          <xsl:when test="ancestor::TEI/teiHeader[1]/fileDesc[1]/publicationStmt[1]/idno[1]/@type='HBAS-E'">
             <xsl:choose>
                <xsl:when test="not(ancestor::quote) and self::workName">
                   <xsl:text>\emph{</xsl:text> 
                  <!-- Titel kursiv wenn sie in Herausgebertexten sind -->
                   <xsl:apply-templates/>
                   <xsl:text>}</xsl:text>
                </xsl:when>
                <xsl:otherwise>
...
keepaspectratio]</xsl:text>
   ...
   <xsl:template match="Zyklus">
      <xsl:variable name="work-entry"
                    select="key('work-lookup', preceding-sibling::Nummer, $works)"/>
      <xsl:variable name="zyklus-entry"
                    select="key('work-lookup', substring(.,1, 7), $works)"/>
      <xsl:choose>
